...
def build_flowchart(out_name="口腔语音电子病历系统流程图_v2", fmt="png"):
    g = Digraph(name="EMR_System", format=fmt)

    # ===== 全局布局（保持你示例那种“标题突出、块清晰”）=====
    g.attr(
        rankdir="TB",
        splines="ortho",
        nodesep="0.30",
    ...
    g.node(
        ...
        "   - 系统性调参并监控训练过程，兼顾识别准确率与提取精确率/召回率",
        style="filled",
        fillcolor="#ffffff",
        fontsize="10.5",
        width="9.6"
    ...
    g.node(
        "S4C",
        "① 客观评估：精确率、召回率、F1；信息覆盖率；文本相似度；术语规范率\n"
        "② 主观评价：种植科医生 5 级 Likert 量表，从完整性、准确性、逻辑一致性等 7 维度盲评\n"
        "③ 幻觉与效率：统计幻觉发生率，记录书写与修改耗时及修改次数\n"
        "④ 消融实验：比较“模板约束+信息提取”与“自由生成式 LLM”的性能差异，验证前者在抑制幻觉与提升规范性中的作用",
        style="filled",
        fillcolor="#ffffff",
        fontsize="10.5",
        width="9.6"
...
if __name__ == "__main__":
    # png / svg / pdf 都可以。论文推荐 svg/pdf
    build_flowchart(fmt="png")
